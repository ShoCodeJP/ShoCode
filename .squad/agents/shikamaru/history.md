...
- 2026-04-01: Sprint 2 — Created `CODEOWNERS` (#42) with @drewby as default owner and specific patterns for profiles/, .github/, .squad/, and ops/. Created `.github/workflows/stale-pr.yml` (#29) using `actions/stale@v9` — targets PRs only (issues disabled), 14-day stale mark, 7-day close window, bilingual EN/JP messages, exempts `keep-open` and `work-in-progress` labels, runs weekly Monday 09:00 UTC.
- 2026-04-09: Built GitHub Pages landing page in `docs/` folder. Approach: plain HTML/CSS/vanilla JS, no build step, no frameworks. Bilingual EN/JP toggle uses `lang` attributes on elements + a CSS visibility pattern (`body.ja [lang="en"] { display: none }` etc.) switched by a single JS IIFE. Respects `localStorage` preference and falls back to `navigator.language`. Serves from `docs/` on main branch — Drew needs to enable Pages in repo Settings. Banner image copied to `docs/images/` for self-contained assets.
- 2026-06-30: Created 11 sponsor labels (5 tier + 6 stage) and 21 prospect issues in private repo `ShoCodeJP/shocode-internal` via GitHub MCP. Projects board needs manual creation due to token scope limitation.
